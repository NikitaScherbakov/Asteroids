...
public class Rock extends Polygon {
    ...
    public void move(SpaceShip spaceShip, ArrayList<ShootTorpedo> shootTorpedos){
        ...
        for(Rock rock : rocks){
            if(rock.onScreen){
                ...
                if(otherRock.intersects(shipBox)){
                    BoardFrame.playSoundEffect(explodeFile);

                    spaceShip.setXCenter(spaceShip.bFWidth/2);
                    spaceShip.setYCenter(spaceShip.bFHeight/2);

                    spaceShip.setxVelocity(0);
                    spaceShip.setyVelocity(0);
                }

                for(ShootTorpedo shootTorpedo : shootTorpedos){
                    if(shootTorpedo.onScreen){
                        if(otherRock.contains(shootTorpedo.getxCenter(), shootTorpedo.getyCenter())){
                            rock.onScreen = false;
                            shootTorpedo.onScreen = false;
        ...
        if(uLeftXPos < 0 || (uLeftXPos + 50) > height){
    ...
    public static int[] getsPolyXArray(int randomStartXPos){
    ...
    public static int[] getsPolyYArray(int randomStartYPos){
